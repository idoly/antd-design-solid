...
test('applies numeric component tokens as valid computed CSS values', async ({ page }, testInfo) => {
  ...
  const avatars = page.getByTestId('state-avatars');
  const standaloneAvatars = avatars.locator(':scope > .ads-avatar');
  await expect(standaloneAvatars.nth(0)).toHaveCSS('width', '36px');
  await expect(standaloneAvatars.nth(0)).toHaveCSS('font-size', '16px');
  await expect(standaloneAvatars.nth(1).locator('[data-avatar-content="icon"]')).toHaveCSS('font-size', '20px');
  await expect(standaloneAvatars.nth(2)).toHaveCSS('width', '26px');
  await expect(standaloneAvatars.nth(2)).toHaveCSS('font-size', '12px');
  await expect(standaloneAvatars.nth(3).locator('[data-avatar-content="icon"]')).toHaveCSS('font-size', '13px');
  await expect(standaloneAvatars.nth(4)).toHaveCSS('width', '48px');
  await expect(standaloneAvatars.nth(4)).toHaveCSS('font-size', '18px');
  await expect(standaloneAvatars.nth(5).locator('[data-avatar-content="icon"]')).toHaveCSS('font-size', '26px');
  const groupedAvatars = avatars.locator('.ads-avatar-group > .ads-avatar');
  await expect(groupedAvatars.nth(0)).toHaveCSS('border-color', 'rgb(247, 215, 116)');
  await expect(groupedAvatars.nth(1)).toHaveCSS('margin-left', '-10px');
  await avatars.locator('[data-avatar-overflow="true"]').click();
  const hiddenAvatars = page.getByRole('dialog', { name: 'Hidden avatars' }).locator('.ads-avatar');
  await expect(hiddenAvatars).toHaveCount(2);
  await expect(hiddenAvatars.nth(1)).toHaveCSS('margin-left', '6px');
