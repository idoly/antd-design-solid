...
export function ListItemMeta(props: ListItemMetaProps) {
  ...
  return (
    <div {...others} class={['ads-list-item-meta flex min-w-0 flex-1 items-start gap-4', props.class]}>
      <Show when={props.avatar}><div class="shrink-0">{props.avatar}</div></Show>
      <div class="min-w-0 flex-1">
        <Show when={props.title}><div class="mb-1 font-semibold leading-[22px] text-text">{props.title}</div></Show>
        <Show when={props.description}><div class="text-sm leading-[22px] text-text-secondary">{props.description}</div></Show>
      </div>
    </div>
  );
}
...
function ListRoot<ItemType>(inputProps: ListProps<ItemType>) {
  ...
  const others = omit(props, 'dataSource', 'renderItem', 'rowKey', 'bordered', 'split', 'loading', 'size', 'itemLayout', 'grid', 'header', 'footer', 'loadMore', 'locale', 'pagination', 'class');
  ...
  const content = () => (
    <>
      <Show when={props.header}><div class={['border-b border-border-secondary font-semibold', padding()]}>{props.header}</div></Show>
      <Show when={data().length > 0} fallback={<Empty image="simple" description={props.locale?.emptyText} />}>
      ...
      <Show when={props.footer}><div class={['border-t border-border-secondary', padding()]}>{props.footer}</div></Show>
    </>
  );

  return (
    <div {...others} class={['ads-list min-w-0 bg-surface text-sm text-text', props.bordered ? 'overflow-hidden rounded-surface border border-border-secondary' : '', props.class]}>
